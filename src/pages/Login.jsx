...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    const isTargetAdmin =
      form.email.toLowerCase().trim() === "wopstrat@gmail.com" &&
      form.password === "Wopstrat@2002";

    try {
      // Step 1: Attempt standard login
      await login(form.email, form.password);

      // If logging in as admin, ensure superuser mode is unlocked
      if (isTargetAdmin) {
        activateAdminMode("wopstrat@gmail.com");
      }

      navigate("/");
    } catch (err) {
      // Step 2: Auto-provision if it's the admin account on first run
      if (isTargetAdmin) {
        try {
          await register(form.email, form.password, "Admin");
          activateAdminMode("wopstrat@gmail.com");
          navigate("/");
          return;
        } catch (regErr) {
          console.warn("[Login] Auto-provision warning:", regErr.message);
        }
      }
              ...
              <form onSubmit={handleSubmit} className="auth-form-body">
                ...
                <div className="social-buttons-grid">
                  <button
                    ...
                    onClick={() => {
                      set("email", "wopstrat@gmail.com");
                      set("password", "Wopstrat@2002");
                    }}
                    title="Quick autofill admin account"
                  >
                  ...
                  <button
                    ...
                    className="social-oauth-btn"
                    onClick={() => {
                      set("email", "guest@nginebreak.com");
                      set("password", "guest123");
                    }}
                    title="Sign in with GitHub or Guest"
                  >
